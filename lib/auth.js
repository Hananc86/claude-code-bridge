...
async function submitCode(name, code) {
  ...
  for (let i = 0; i < 50; i++) {           // ~25 s
    ...
    if (txt.includes("Login successful")) {
      flows.delete(name);
      // The SAME resolver the flow was started from. A second lookup that did
      // not know about the ambient login would report `status: null` after a
      // renewal that genuinely succeeded — the operator renews, and the one
      // number they came for is missing.
      const a = renewable().list.find(x => x.name === name);
