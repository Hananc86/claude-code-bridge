...
// ── how a flow ENDED, kept after its row is gone ─────────────────────────────
//
// ⚠️ THE DEFECT THIS CLOSES, measured on work-laptop 2026-09-24. The CLI opened
// the sign-in page itself, its own localhost callback completed the login, and
// `reap()` settled the flow and deleted its row — correct, and the thing that
// cleared the router's auth block. The operator's pasted code then arrived at
// an EMPTY TABLE and was answered:
//
//     no login is waiting for this account — start one first
//
// in red, as the last thing on screen, under a renewal that had just SUCCEEDED
// (expiry moved 2026-10-22 -> 2026-10-24). That sentence is true for a flow
// that never started, and it tells an operator whose login is finished to go
// and do it again.
//
// `reap()` already KNEW which it was — it had just called noteLoginSucceeded on
// the line above — and threw the answer away. ONE SENTENCE FOR FOUR FACTS is
// the shape this file keeps paying for; the fact is recorded instead.
const SETTLED_TTL_MS = 15 * 60 * 1000;
const SETTLED_MAX = 16;
const settled = new Map();   // account name -> {outcome, at, credBefore, account}

// ONE home for "this flow is over", so an outcome cannot be forgotten at one of
// the sites that ends one — the `queueTarget` / `maMergeQueue` twin shape. Only
// `startFlow`'s supersede path deletes a row without coming through here, and
// it CLEARS the record rather than writing one (see there); §[5] pins that.
function endFlow(name, outcome, f) {
  flows.delete(name);
  settled.delete(name);        // re-insert, so the bound below evicts by recency
  settled.set(name, { outcome, at: Date.now(),
                      credBefore: f ? f.credBefore : null,
                      account: f ? f.account : null });
  while (settled.size > SETTLED_MAX) settled.delete(settled.keys().next().value);
}

// ⚠️ A `succeeded` RECORD IS NOT TAKEN ON ITS OWN WORD. It is re-verified
// against the credential before it is repeated to the operator, because the two
// mistakes are not symmetric: an unhelpful "start one first" costs a round
// trip, while telling someone their login is fine when it is not sends them
// away from a dead account. A record that no longer agrees with the credential
// is DOWNGRADED to no answer, never upgraded.
function settledOutcome(name) {
  const rec = settled.get(name);
  if (!rec) return null;
  if (Date.now() - rec.at > SETTLED_TTL_MS) { settled.delete(name); return null; }
  if (rec.outcome !== "succeeded") return { outcome: rec.outcome, account: null };
  const a = renewable().list.find(x => x.name === name);
  if (!a || !credAdvanced(rec.credBefore, credFingerprint(a))) return null;
  return { outcome: "succeeded", account: a };
}

// Four facts, four sentences. Each names the remedy, because "start one first"
// after a 30-minute timeout and after a CLI that died are the same words for
// two different problems.
const ENDED_MSG = {
  expired: "that login timed out after 30 minutes and was closed — start a new one",
  exited: "that login ended on its own without succeeding — start a new one",
  cancelled: "that login was cancelled — start a new one",
  none: "no login is waiting for this account — start one first",
};
...
// ONE home for "this login succeeded", called from every site that declares
// one. There are FOUR — reap(), startFlow's browser-callback exit, and both of
// submitCode's success branches — and a fix applied to two of them is the
// shape this file keeps paying for (`queueTarget`, the `maMergeQueue` twin,
// the usage-limit branch one `elif` from the branch that actually fired).
// §[4] pins the count rather than the call sites, so a FIFTH success path
// fails the suite instead of quietly inheriting the stale verdict.
//
// ⚠️ It clears on EITHER confirmation, not only on `advanced`. The product has
// exactly one definition of a successful login — the `ok: true` it returns to
// the operator — and the CLI's own "Login successful" is one of the two
// signals it accepts. Clearing only on the credential signal would leave the
// block standing on precisely the platforms where the prose fires first, i.e.
// reproduce the defect everywhere except Windows.
//
// ⚠️ Same process as the router, so this is an ordinary in-process
// read-modify-write on state the bridge already owns — NOT the cross-process
// hazard that keeps the PVE auth console read-only.
function noteLoginSucceeded(name, how) {
  try { accounts.noteCredentialRenewed(name); } catch (_e) {}
  console.error("[auth] login OK for %s (%s) — cleared any auth block", name, how);
}
...
function reap() {
  ...
  for (const [name, f] of [...flows]) {
    ...
    if (f.account && credAdvanced(f.credBefore, credFingerprint(f.account))) {
      // ⚠️ RECORD WHICH IT WAS. This branch is the one that ran on work-laptop,
      // and the operator's paste landed a moment later with no way to learn
      // that the thing they were pasting for had already happened.
      endFlow(name, "succeeded", f);
      // ⚠️ CLOSE STDIN, DO NOT SIGNAL. Deleting the row without touching the
      // process would orphan it: nothing else will ever reclaim a flow that is
      // no longer in the table, and a CLI blocked on a read it will never get
      // holds that pipe for as long as the bridge runs. EOF is the gentle exit
      // — a process blocked on stdin returns, and one mid-write is unaffected.
      // SIGTERM here is what turns a successful login into a corrupt
      // credential file, which is the whole reason this branch does not kill.
      try { if (f.proc.stdin) f.proc.stdin.end(); } catch (_e) {}
      noteLoginSucceeded(name, "credential advanced");
      continue;
    }
    // A TTL lapse and a CLI that died are two different problems with two
    // different remedies, so they are not collapsed into one word here.
    const timedOut = now - f.started > FLOW_TTL_MS;
    if (timedOut || f.proc.exitCode !== null || f.proc.signalCode) {
      if (f.proc.exitCode === null && !f.proc.signalCode) killFlow(f);
      endFlow(name, timedOut ? "expired" : "exited", f);
    }
  }
  // Records outlive their flows only briefly; prune here rather than leaving
  // them to the bound, so a stale `succeeded` cannot answer a much later paste.
  for (const [name, rec] of [...settled]) {
    if (now - rec.at > SETTLED_TTL_MS) settled.delete(name);
  }
}
...
async function startFlow(name, claudeBin) {
  ...
  // ⚠️ THE ONLY DELETE THAT DOES NOT RECORD AN OUTCOME, AND IT CLEARS INSTEAD.
  // A new login makes every previous verdict history: leaving a `succeeded`
  // record here would let it answer a paste belonging to THIS flow after this
  // one is reaped, reporting an old success as if it were the new one.
  settled.delete(name);
  ...
  if (done) {
    noteLoginSucceeded(name, "browser callback during start");
...
async function submitCode(name, code) {
  ...
  if (!f) {
    const s = settledOutcome(name);
    if (s && s.outcome === "succeeded") {
      // ⚠️ NOT AN ERROR. The operator wanted a renewed login and they have one;
      // returning `ok:false` here is the same inversion this file already
      // refuses on the exit race below — "reporting a successful login as a
      // failed one is the worse error of the two" — one surface further out.
      // The status is READ from disk, so this reports the real new expiry and
      // never an assumed one.
      //
      // ⚠️ IT ANNOUNCES THE LOGIN LIKE EVERY OTHER SUCCESS RETURN, and the
      // auth-block suite's structural pin is what caught it missing here — a
      // FIFTH success path quietly inheriting the stale verdict is the exact
      // shape that pin exists for. The site that recorded this outcome had
      // already announced it, so this is idempotent rather than necessary —
      // but "the other site always does it" is an argument, and the pin is a
      // guarantee.
      noteLoginSucceeded(name, "settled before the paste arrived");
      return { ok: true, status: credStatus(s.account), confirmed_by: "settled_before_paste",
               note: "That login had already completed on its own — the sign-in page " +
                     "finished it, so the code was not needed." };
    }
    return { ok: false, error: ENDED_MSG[(s && s.outcome)] || ENDED_MSG.none };
  }
  ...
  for (let i = 0; i < 50; i++) {           // ~25 s
    ...
    if (advanced || txt.includes("Login successful")) {
      endFlow(name, "succeeded", f);
      noteLoginSucceeded(name, advanced ? "credential" : "cli_output");
    ...
    if (f.proc.exitCode !== null) {
      ...
      if (a2 && credAdvanced(f.credBefore, credFingerprint(a2))) {
        endFlow(name, "succeeded", f);
        noteLoginSucceeded(name, "credential, seen after exit");
        return { ok: true, status: credStatus(a2), confirmed_by: "credential" };
      }
      endFlow(name, "exited", f);
...
module.exports = {
  ...
  FLOW_TTL_MS, SETTLED_TTL_MS, CODE_RE, URL_RE, ENDED_MSG,
  _flows: flows,
  _settled: settled,
};
