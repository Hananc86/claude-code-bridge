...
const authFlow = require("./auth");   // NOT `auth`: the request handler
                                      // binds `auth` to the bearer header.
...
// The CLI writes a session to <CONFIG_DIR>/projects/<encoded cwd>/<id>.jsonl,
// and an ACCOUNT IS A CONFIG DIR — so with routing on, one machine has SEVERAL
// transcript stores while this used to be able to see exactly one of them (a
// hardcoded ~/.claude/projects, which did not even honour an ambient
// CLAUDE_CONFIG_DIR).
//
// That blindness is what produced `Session ID <id> is already in use`: the
// resume-vs-create decision was taken ONCE, against the default store, while
// each failover attempt ran under a DIFFERENT store — the CLI created the
// transcript there and then failed, and the next attempt to land on a store
// that already held the id was refused by the CLI outright. Measured on
// work-laptop 2026-09-24: the same session file in FOUR directories, with
// acct2/3/4 holding exactly one transcript each, created in the minute of the
// failure.
//
// ONE home for the answer, consumed by every read, the delete and the spawn
// decision, so they cannot disagree about where a session is.
function configRoot() {
  return process.env.CLAUDE_CONFIG_DIR || path.join(homeDir(), ".claude");
}
function projectsDirFor(configDir) {
  return path.join(configDir || configRoot(), "projects");
}
// Every store this machine could have written a transcript to, most-canonical
// first: the ambient one (what the CLI uses with no override), the historical
// default, then every configured account. Deduped through realpath, because
// the PVE convention symlinks every account's projects/ at ONE physical store
// — there this collapses back to a single root and the whole thing is a no-op,
// which is exactly why that host never saw the defect.
//
// The order matters: the ambient/default root is first, so any session that
// was visible before this existed answers identically. The union only ever
// ADDS transcripts that were previously unreachable.
function sessionRoots() {
  ...
  const add = (d) => {
    if (!d) return;
    let real = d;
    try { real = fs.realpathSync(d); } catch { /* absent: keep the literal */ }
    if (seen.has(real)) return;
    seen.add(real);
    out.push(d);
  };
  add(projectsDirFor(configRoot()));
  add(path.join(homeDir(), ".claude", "projects"));
  try {
    const cfg = accounts.loadConfig();
    for (const a of (cfg.all || cfg.accounts || [])) add(projectsDirFor(a.config_dir));
  } catch {}
  return out;
}
// Does the store THIS attempt will write to already hold this session? The
// spawn path asks per attempt and per account, and deliberately NOT through
// sessionRoots(): a transcript in some OTHER account's store is not something
// `--resume` can open under this account, and answering from the union would
// turn "already in use" into "No conversation found" — the same defect wearing
// the other error message.
function sessionFileInRoot(base, sessionId) {
  let dirs;
  ...
  for (const base of sessionRoots()) {
    ...
    for (const project of dirs) {
      ...
      for (const file of files) {
        ...
        // One row per session. A failover that created the id in several
        // stores would otherwise list the same conversation once per account,
        // and the earlier root is the one every previous build answered with.
...
function findSessionFile(sessionId) {
  for (const base of sessionRoots()) {
    const fp = sessionFileInRoot(base, sessionId);
    if (fp) return fp;
  }
  return null;
}
// EVERY copy, for the delete paths. A delete that removed one store's copy
// while the listing unions all of them would report success and leave the
// session on screen — so removal has to cover exactly what reading covers.
function findSessionFilesAll(sessionId) {
  const out = [];
  for (const base of sessionRoots()) {
    const fp = sessionFileInRoot(base, sessionId);
...
function deleteSession(sessionId) {
  ...
  for (const [cid, e] of Object.entries(agents)) {
    if (e.parent === sessionId) {
      for (const fp of findSessionFilesAll(cid)) { try { fs.unlinkSync(fp); } catch {} }
      delete agents[cid]; cascade.push(cid);
    }
  }
  delete agents[sessionId];
  saveAgents(agents);
  const targets = findSessionFilesAll(sessionId);
  if (!targets.length) return { error: "not found" };
  for (const filePath of targets) { try { fs.unlinkSync(filePath); } catch {} }
...
function precompactIfNeeded(config, { session_id, model, cwd, fork }) {
  return new Promise((resolve) => {
    ...
    args.push("/compact");
    const tgt = claudeSpawn.buildSpawn(config.claudeBin, args);
    const t0 = Date.now();
    let done = false;
    const finish = (result) => {
    ...
    // `--resume` can only open a transcript in the store it lives in, and the
    // union above can now find one in ANY account's store — so compact it where
    // it is, rather than spawning against the ambient store and logging a
    // "failed" pre-compaction for a session that was never there.
    // <configDir>/projects/<project>/<id>.jsonl — three levels up is the store.
    const env = { ...process.env };
    try {
      const owner = path.dirname(path.dirname(path.dirname(existing.filePath)));
      // Only when it differs: a machine with no routing must keep passing its
      // own environment through untouched.
      if (owner && owner !== configRoot()) env.CLAUDE_CONFIG_DIR = owner;
    } catch {}
    let proc;
    try {
      proc = spawn(tgt.bin, tgt.args,
        { cwd: cwd || process.cwd(), env,
...
function askClaudeInner(config, { prompt, session_id, images, cwd, plan_mode, allow_tools, model, fork }) {
  return new Promise((resolve) => {
    ...
    // ⚠️ This used to be its own `.endsWith(".cmd")`, which missed `.bat` and
    // `.CMD` while the pre-compaction path six hundred lines up tested
    // `/\.(cmd|bat)$/i` — two spellings of one fact, disagreeing. On a `.bat`
    // shim the weaker test made this path skip the @file prompt AND spawn the
    // shim directly, i.e. EINVAL. One home now.
    const isCmdShim = claudeSpawn.isCmdShim(config.claudeBin);
    ...
    // ⚠️ The session flags are deliberately NOT pushed here. They are decided
    // PER ATTEMPT (see sessionArgsFor below), because each failover attempt
    // runs under a different CLAUDE_CONFIG_DIR and "does this session already
    // exist" is a different question in each one. Deciding it once — against a
    // store the later attempts never touch — is what produced `Session ID … is
    // already in use`.
    //
    // This index is where they used to go, and where they still go: argument
    // ORDER stays byte-for-byte what it was. On a Windows cmd shim the TRAILING
    // arguments are the fragile ones (a newline once truncated the line and
    // silently dropped --resume, so every resume started a new session), and
    // --resume is not a flag to move to the end for tidiness.
    const sessionArgAt = args.length;
    if (session_id) {
      // cwd is a property of the SESSION, not of the account that served it,
      // so this one legitimately reads the union of every store.
    ...
    // (That flattening now lives in ./claude-spawn, with the rest of the rule.)

    const bin = claudeSpawn.spawnBin(config.claudeBin);
    ...
    //
    // The SESSION flag is rebuilt per attempt for the same reason, against the
    // store the attempt will actually write to: a store that already holds the
    // id can only be RESUMED (the CLI refuses to create it there — the reported
    // defect), and a store that does not hold it can only be CREATED.
    //
    // Deliberately NOT asked of sessionRoots(): a transcript sitting in some
    // OTHER account's store is not something `--resume` can open under this
    // account, so answering from the union would just trade "already in use"
    // for "No conversation found".
    const sessionArgsFor = (account) => {
      if (!session_id) return [];
      const base = projectsDirFor(account ? account.config_dir : null);
      if (sessionFileInRoot(base, session_id)) {
        const a = ["--resume", session_id];
    ...
    const buildSpawnArgs = (modelOverride, account) => {
      const a = args.slice(0, sessionArgAt)
                    .concat(sessionArgsFor(account), args.slice(sessionArgAt));
      if (modelOverride) {
        const mi = a.indexOf("--model");
        if (mi !== -1) a[mi + 1] = modelOverride;
        else a.push("--model", modelOverride);
      }
      const t = claudeSpawn.buildSpawn(config.claudeBin, a);
      if (t.flattened && process.env.CLAUDE_BRIDGE_DEBUG) {
        console.error("[bridge] flattened newlines in %d arg(s) for cmd.exe", t.flattened);
      }
      return t.args;
    };
    ...
    const runOnce = () => new Promise((done) => {
      ...
      const spawnArgs = buildSpawnArgs(curModel, acct);
      // Opt-in spawn tracing (CLAUDE_BRIDGE_DEBUG=1). Nothing recorded WHAT the
      // bridge actually ran, so when a turn misbehaved there was no evidence to
      // separate "we built the wrong command" from "the CLI ignored it" — a
      // resume regression took hours to narrow with no way to see the argv.
...
function startBridge(config) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // ── Login renewal ───────────────────────────────────────────────────────
    // The login runs HERE and the token is created HERE. Only the operator's
    // one-time code crosses the wire, and only inward. Nothing in this surface
    // returns a token, a fragment of one, or a config-dir path — and the
    // account is always chosen BY NAME from this machine's own accounts file,
    // never by a path supplied in the request.
    if (url.pathname === "/auth/accounts" && req.method === "GET") {
      try { send(200, authFlow.listAccounts()); }
      catch (e) { send(500, { error: e.message || String(e) }); }
      return;
    }

    if (url.pathname === "/auth/login/start" && req.method === "POST") {
      // readBody already returns a PARSED object. An extra JSON.parse here
      // stringifies it to "[object Object]", throws, and silently yields {} —
      // so every valid request would be answered "account is required".
      const body = await readBody(req) || {};
      const name = typeof body.account === "string" ? body.account : "";
      if (!name) { send(400, { ok: false, error: "account is required" }); return; }
      try {
        const r = await authFlow.startFlow(name, config.claudeBin);
        send(r.ok ? 200 : 400, r);
      } catch (e) { send(500, { ok: false, error: e.message || String(e) }); }
      return;
    }

    if (url.pathname === "/auth/login/code" && req.method === "POST") {
      // readBody already returns a PARSED object. An extra JSON.parse here
      // stringifies it to "[object Object]", throws, and silently yields {} —
      // so every valid request would be answered "account is required".
      const body = await readBody(req) || {};
      ...
      if (!name) { send(400, { ok: false, error: "account is required" }); return; }
      try {
        const r = await authFlow.submitCode(name, body.code);
        send(r.ok ? 200 : 400, r);
      } catch (e) { send(500, { ok: false, error: e.message || String(e) }); }
      return;
    }
    ...
    if (url.pathname === "/auth/login/cancel" && req.method === "POST") {
      ...
      const name = typeof body.account === "string" ? body.account : "";
      if (!name) { send(400, { ok: false, error: "account is required" }); return; }
      try { send(200, authFlow.cancelFlow(name)); }
      catch (e) { send(500, { ok: false, error: e.message || String(e) }); }
      return;
    }
