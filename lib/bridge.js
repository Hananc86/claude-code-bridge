...
function askClaude(config, { prompt, session_id, images, cwd, plan_mode, allow_tools }) {
  return new Promise((resolve) => {
    ...
    const spawnBin = isWin && config.claudeBin.endsWith(".cmd") ? process.env.COMSPEC || "cmd.exe" : config.claudeBin;
    const spawnArgs = isWin && config.claudeBin.endsWith(".cmd") ? ["/c", config.claudeBin, ...args] : args;
